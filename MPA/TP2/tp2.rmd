...
res1 <- algorithme_gibbs(y, n_iteration = 10000, n_elimine = 1000,
                         c_initial = 1, theta1_initial = 0.1, theta2_initial = 0.9)

res2 <- algorithme_gibbs(y, n_iteration = 10000, n_elimine = 1000,
...
res1 <- algorithme_gibbs(seq1, n_iteration = 10000, n_elimine = 1000,
...
```{r}
res1Gauche <- algorithme_gibbs(seq1[1:(point_estime-1)], n_iteration = 10000,
                               n_elimine = 1000, 
...
plot(res2$theta2_samples, res2$c_samples, col = "darkgreen",
     ...
     pch = 16, cex = 0.6)
```
1. Corrélation $\theta_1$ - $\theta_2$ :
Le nuage de points (en rouge) montre une corrélation faible 
Cela est cohérent avec le modèle :
$\theta_1$ représente la probabilité avant le point de changement,
$\theta_2$ celle après.

Si la proportion avant augmente, celle après doit logiquement baisser.
Le nuage montre qu'il n'a pas de dépendance forte : bonne exploration conjointe de l’espace de paramètres. Bon signe pour la convergence.
